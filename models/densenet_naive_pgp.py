...
from .densenet_naive import BuildingDenseBlock
...
class Transition(chainer.Chain):
    ...
    def __call__(self, x):
        h = F.relu(self.bn(x))
        h = self.conv(h)
        h = F.average_pooling_2d(pgp(h, 2), self.stride)
        # h = pgp(F.average_pooling_2d(x, 2, 1, 1)[:, :, 1:, 1:], 2)
        return h


class DenseNet_PGP(chainer.Chain):
